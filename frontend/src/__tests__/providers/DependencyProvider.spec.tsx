...
function wrapper(overrides: ReturnType<typeof makeOverrides>) {
  return function Wrapper({ children }: { children: React.ReactNode }) {
    return (
      <DependencyProvider overrides={overrides}>{children}</DependencyProvider>
    );
  };
}
...
describe("DependencyProvider", () => {
  it("throws when useDeps is called outside the provider", () => {
    ...
    expect(() => renderHook(() => useDeps())).toThrow(
      "DependencyProvider is required"
    );
  ...
  it("partial overrides merge with defaults without crashing", () => {
    ...
    const partialWrapper = ({ children }: { children: React.ReactNode }) => (
      <DependencyProvider overrides={partialOverride}>
        {children}
      </DependencyProvider>
    );

    expect(() =>
      renderHook(() => useDeps(), { wrapper: partialWrapper })
    ).not.toThrow();
  });
});
