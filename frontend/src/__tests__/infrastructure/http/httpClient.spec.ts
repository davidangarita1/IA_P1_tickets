...
describe('httpClient', () => {
  ...
  describe('httpGet', () => {
    ...
    it('throws CIRCUIT_OPEN when circuit breaker is open', async () => {
      ...
      await expect(httpGet('http://localhost:3000/test')).rejects.toThrow('CIRCUIT_OPEN');
    ...
    it('throws RATE_LIMIT on 429 status without retry', async () => {
      ...
      await expect(httpGet('http://localhost:3000/test')).rejects.toThrow('RATE_LIMIT');
    });
    ...
    it('throws SERVER_ERROR on 500+ status after retries', async () => {
      ...
      await expect(httpGet('http://localhost:3000/test')).rejects.toThrow('SERVER_ERROR');
    ...
    it('throws HTTP_ERROR on 4xx status', async () => {
      ...
      await expect(httpGet('http://localhost:3000/test')).rejects.toThrow('HTTP_ERROR');
    });
  });
  ...
  describe('httpPost', () => {
    it('sends JSON body and returns parsed response', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ status: 'accepted', message: 'Created' }),
      });
  ...
  describe('retries', () => {
    ...
    it('throws TIMEOUT when request is aborted at last retry', async () => {
      ...
      await expect(httpGet('http://localhost:3000/test', { retries: 0 })).rejects.toThrow(
        'TIMEOUT',
      );
    });
    ...
    it('throws original error when non-Error rejection exhausts retries', async () => {
      ...
      await expect(httpGet('http://localhost:3000/test', { retries: 0 })).rejects.toBe(
        'plain string error',
      );
    });
  });
