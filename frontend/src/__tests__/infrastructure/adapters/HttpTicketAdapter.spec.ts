...
const mockedToDomain = toDomainTicket as jest.MockedFunction<typeof toDomainTicket>;
const mockedToBackend = toBackendCreateDTO as jest.MockedFunction<typeof toBackendCreateDTO>;
...
describe('HttpTicketAdapter', () => {
  ...
  describe('getTickets()', () => {
    it('calls httpGet with correct URL and maps results', async () => {
      ...
      expect(mockedHttpGet).toHaveBeenCalledWith('http://localhost:3000/turnos');
  ...
  describe('createTicket()', () => {
    it('maps DTO through ACL and calls httpPost', async () => {
      ...
      expect(mockedHttpPost).toHaveBeenCalledWith('http://localhost:3000/turnos', backendDTO);
    ...
    it('propagates errors from httpPost', async () => {
      ...
      await expect(adapter.createTicket({ name: '', documentId: 0 })).rejects.toThrow('TIMEOUT');
    });
  });
});
