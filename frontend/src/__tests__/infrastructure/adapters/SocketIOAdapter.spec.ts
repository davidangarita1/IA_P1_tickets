...
describe('SocketIOAdapter', () => {
  ...
  it('calls onConnect callback on connect event', () => {
    ...
    const connectHandler = mockOn.mock.calls.find(([event]: [string]) => event === 'connect')?.[1];
    connectHandler?.();
  ...
  it('calls onDisconnect callback on disconnect event', () => {
    ...
    const handler = mockOn.mock.calls.find(([event]: [string]) => event === 'disconnect')?.[1];
    handler?.();
  ...
  it('calls onError on connect_error event', () => {
    ...
    const handler = mockOn.mock.calls.find(([event]: [string]) => event === 'connect_error')?.[1];
    handler?.();

    expect(callbacks.onError).toHaveBeenCalledWith('Error de conexión con el servidor');
  });

  it('maps TURNOS_SNAPSHOT data through toDomainTicket', () => {
    const mockedMapper = toDomainTicket as jest.MockedFunction<typeof toDomainTicket>;
    mockedMapper.mockImplementation((raw) => raw as unknown as ReturnType<typeof toDomainTicket>);

    adapter.connect(callbacks);

    const handler = mockOn.mock.calls.find(([event]: [string]) => event === 'TURNOS_SNAPSHOT')?.[1];
    ...
    expect(callbacks.onSnapshot).toHaveBeenCalledWith([{ id: '1' }, { id: '2' }]);
  });
