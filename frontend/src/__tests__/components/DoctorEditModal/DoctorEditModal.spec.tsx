...
const mockUseAvailableShifts = useAvailableShifts as jest.MockedFunction<
  typeof useAvailableShifts
>;
...
describe("DoctorEditModal", () => {
  ...
  it("renders the modal title as Editar Medico", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /editar médico/i })
    ).toBeInTheDocument();
  });
  ...
  it("calls doctorService.update on submit with changed fields", async () => {
    const doctor = buildDoctor({ _id: "doc-1", name: "Juan García", documentId: "12345678", office: "2", shift: "06:00-14:00" });
    ...
    await waitFor(() => {
      expect(service.update).toHaveBeenCalledWith("doc-1", expect.objectContaining({ name: "Pedro López" }));
  ...
  it("shows error toast on failed update", async () => {
    const doctor = buildDoctor();
    const service = mockDoctorService([doctor]);
    service.update.mockRejectedValueOnce(new Error("La franja horaria del consultorio ya está ocupada"));
    const showToast = jest.fn();
    ...
    await waitFor(() => {
      expect(showToast).toHaveBeenCalledWith("La franja horaria del consultorio ya está ocupada", "error");
    });
  });
