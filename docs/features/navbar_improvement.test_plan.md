...
- **HU-01:** Renombramiento del botón "Registrar" a "Request Turn", cambio de ruta `/register` → `/request-turn`, habilitación del botón en la barra de navegación pública para usuarios no autenticados y redirección de la ruta anterior.
...
- Verificación de renombramiento de botón "Registrar" → "Request Turn" en todos los puntos donde aparece.
- Verificación de cambio de ruta `/register` → `/request-turn`.
- Verificación de redirección automática de `/register` a `/request-turn`.
- Visibilidad de la barra de navegación pública (botones "Request Turn", "Turnos", "Iniciar Sesión") para usuarios no autenticados.
...
- Pantallas destino (contenido de `/request-turn`, `/dashboard`, `/`): solo se verifica que la navegación llega al destino correcto, no la funcionalidad interna de cada pantalla.
...
|------------|---------------------|
| HU-01: Renombramiento del botón "Registrar" y habilitación en la barra de navegación pública | - El botón "Registrar" se renombra a "Request Turn" en todos los puntos donde aparece<br>- La ruta cambia de `/register` a `/request-turn`<br>- La pantalla de destino permanece sin cambios<br>- La barra de navegación pública es visible para usuarios no autenticados con los botones "Request Turn" e "Iniciar Sesión"<br>- Clic en "Request Turn" navega a `/request-turn`<br>- La ruta `/register` redirige automáticamente a `/request-turn`<br>- El botón muestra estilo activo en `/request-turn`<br>- La barra pública no es visible para usuarios autenticados |
| HU-02: Visibilidad del botón "Turnos" en la barra de navegación sin autenticación | - El botón "Turnos" es visible para usuarios no autenticados junto a "Request Turn" e "Iniciar Sesión"<br>- Clic en "Turnos" navega a la ruta raíz `/`<br>- El logo y el botón "Turnos" coexisten como elementos de navegación independientes<br>- El botón muestra estilo activo en la ruta `/`<br>- El botón permanece visible para usuarios autenticados |
...
**HU-01 — Renombramiento y habilitación de "Request Turn"**

| ID | Escenario | Precondición | Pasos | Resultado Esperado |
|----|-----------|-------------|-------|-------------------|
| TC-01 | Verificar renombramiento del botón | Usuario no autenticado | 1. Acceder a cualquier página pública | El botón muestra "Request Turn" (no "Registrar") |
| TC-02 | Navbar pública visible sin autenticación | Usuario no autenticado | 1. Acceder a la ruta `/` | La barra muestra "Request Turn", "Turnos" e "Iniciar Sesión" |
| TC-03 | Navegación al hacer clic en "Request Turn" | Usuario no autenticado, navbar visible | 1. Clic en "Request Turn" | La URL cambia a `/request-turn` y se muestra la pantalla de solicitud de turno |
| TC-04 | Redirección de ruta antigua `/register` | Usuario accede directamente a `/register` | 1. Escribir `/register` en el navegador | El sistema redirige a `/request-turn` con la funcionalidad intacta |
| TC-05 | Estilo activo en botón "Request Turn" | Usuario en `/request-turn` | 1. Observar la barra de navegación | El botón "Request Turn" muestra estilo visual activo/resaltado |
...
| TC-19 | Botón "Request Turn" no muestra estilo activo en otra ruta | Usuario en `/` (ruta raíz) | 1. Observar la barra | El botón "Request Turn" NO muestra estilo activo (solo debe activarse en `/request-turn`) |
| TC-20 | Botón "Turnos" no muestra estilo activo en ruta distinta a `/` | Usuario en `/request-turn` | 1. Observar la barra | El botón "Turnos" NO muestra estilo activo |
...
| TC-24 | Doble clic rápido en botón de navegación | Usuario no autenticado | 1. Clic doble rápido en "Request Turn" | El sistema navega correctamente una sola vez, sin errores ni duplicación de navegación |
...
| **Pruebas de Integración (Automatizadas)** | Validar la redirección de `/register` → `/request-turn` y la navegación entre rutas | Jest + Next.js Router mocking |
...
|----|------|---------------|
| TC-01 | Verificación | Confirma conformidad con el criterio de spec: el botón debe llamarse "Request Turn" en todos los puntos |
...
| TC-04 | Verificación | Confirma que la redirección técnica de `/register` → `/request-turn` está implementada correctamente |
| TC-05 | Verificación | Confirma el criterio de spec: el estilo activo se aplica sobre la ruta `/request-turn` |
...
| TC-19 | Verificación | Confirma la exclusividad del estilo activo: "Request Turn" no se resalta en rutas distintas a `/request-turn` |
...
|------------------|-------------------|--------------|------------|------------|
| Usuarios con la ruta `/register` guardada en favoritos no llegan al destino | 3 | 4 | 12 — Alto | Implementar y verificar la redirección 301/302 de `/register` → `/request-turn` (TC-04) |
| Botón "Request Turn" no aparece en navegadores específicos (Safari, mobile) | 2 | 4 | 8 — Medio | Ejecutar pruebas cross-browser y responsivas (TC-02, TC-07) |
...
| Conflicto de ruta al coexistir `/register` (redirect) y `/request-turn` (destino) | 2 | 4 | 8 — Medio | Validar que la redirección funciona sin loops y que la ruta original no renderiza contenido propio |
